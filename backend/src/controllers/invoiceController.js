  ...
  try {
    ...
    if (tipo_ncf) {
      ...
      if (hoy > vencimiento) {
        ...
        return res.status(400).json({
          success: false,
          message: `El rango de NCF tipo ${tipo_ncf} está vencido. Fecha de vencimiento: ${vencimiento.toLocaleDateString('es-DO')}. Solicite un nuevo rango a la DGII.`,
        });
      }
    ...
    for (const item of items) {
      ...
      const product = await Product.findByPk(item.producto_id, {
        transaction,
        lock: transaction.LOCK.UPDATE, // Bloqueo pesimista
      });
      ...
      if (product.stock_actual < item.cantidad) {
        ...
        return res.status(400).json({
          success: false,
          message: `Stock insuficiente para ${product.nombre}. Disponible: ${product.stock_actual}, Solicitado: ${item.cantidad}`,
        });
      }
    ...
    for (const detail of invoiceDetails) {
      ...
      const product = await Product.findByPk(detail.producto_id, {
        transaction,
      });
...
export const anularInvoice = async (req, res) => {
  // Función 'anularInvoice' correctamente exportada
  const transaction = await sequelize.transaction();
  ...
  try {
    ...
    if (invoice.tipo_venta === 'credito') {
      const customer = await Customer.findByPk(invoice.cliente_id, {
        transaction,
      });
  ...
  try {
    ...
    const stats = await Invoice.findAll({
      ...
      attributes: [
        ...
        [
          sequelize.fn('SUM', sequelize.col('balance_pendiente')),
          'balance_pendiente',
        ],
      ],
      raw: true,
